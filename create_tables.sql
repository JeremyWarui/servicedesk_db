...
CREATE TABLE Status (
    ...
    status_type TEXT CHECK (status_type IN ('open', 'in_progress', 'pending', 'resolved')),
...
BEGIN
    ...
    SET
        ...
        resolved_at = CASE
            WHEN (SELECT status_type FROM Status WHERE id = NEW.status_id) = 'resolved' THEN CURRENT_TIMESTAMP
            ELSE resolved_at
        END
